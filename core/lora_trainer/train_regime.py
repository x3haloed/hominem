#!/usr/bin/env python3
"""
Train a 7-regime soft classifier from labeled unified-theory shards.

Assumptions:
- Input JSONL records follow the shard schema with `history`, `target`, and `labels`.
- History is clamped to the last few turns (as produced by the generator).
- Labels contain regime probabilities: regime_support, regime_conflict, regime_problem_solving,
  regime_truth_seeking, regime_crisis, regime_play, regime_boundary.

This is a minimal Trainer-based entry point to get a regime classifier checkpoint.
"""
...
def record_to_text(rec: Dict) -> str:
    ...
    # Clamp history to last 3 turns as per spec
...
def build_dataset(records: List[Dict]) -> Dataset:
    ...
    for rec in records:
        ...
        # Convert regime probabilities to numpy array
        labels.append([float(lbl[k]) for k in REGIME_KEYS])
    if not texts:
...
def main() -> None:
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        problem_type="regression",  # Soft probabilities
